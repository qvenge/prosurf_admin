...
import { Modal, Button, Select, TextInput, ClientSearchInput } from '@/shared/ui';
import {
...
import styles from './CertificateFormModal.module.scss';

interface CertificateFormModalProps {
  certificateId: string | null; // null for create, string for edit
  onClose: () => void;
}

const typeOptions = [
  { value: 'denomination', label: 'Денежный' },
  { value: 'passes', label: 'На посещение' },
];
...
export function CertificateFormModal({ certificateId, onClose }: CertificateFormModalProps) {
  ...
  const handleCreate = () => {
    const data: AdminCreateCertificateDto = {
      ...
      ...(type === 'denomination' && amount
        ...
        : {}),
      ...(type === 'passes' && passes ? { passes: parseInt(passes, 10) } : {}),
          ...
          {isEditing ? (
            ...
            <>
              ...
              {type === 'denomination' ? (
                ...
                <div className={styles.field}>
                  <span className={styles.label}>Количество посещений</span>
                  <TextInput
                    type="number"
                    value={passes}
